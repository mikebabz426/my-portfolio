const projectsArr = [
  {
    name: "Sunset Canines",
    description:
      "Sunset Canines is a company out of Arizona which specializes in providing proffessional pet training around the country.  They reached out to me to see if I would be able to assist with establishing a web presence for their company.  Due to their minimalistic needs, we decided on an informational site using GatsbyJS and optimizing for functionality and speed.",
    url: "https://sunsetcanines.netlify.app/",
    git: "https://github.com/mikebabz426/sunset-canines",
  },
  {
    name: "My Fleet Tracker",
    description:
      "As a veteran of the transportation industry, I have had years of experience working with sub-par software in my day to day job.  One of the reasons I began learning web development is so that I would have the ability to build better software that I would be able to use myself in the logistics sector.  MyFleetTracker is my first such application, designed to increase the productivity of dispatchers and minimize communication issues between colleagues.  ",
    url: "https://speed-myfleettracker.herokuapp.com/",
    git: "https://github.com/mikebabz426/FleetTracker",
  },
]

export default projectsArr
